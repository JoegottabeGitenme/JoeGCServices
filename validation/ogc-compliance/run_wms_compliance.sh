#!/bin/bash
#
# OGC WMS 1.3.0 Compliance Test Runner
#
# This script runs the official OGC ETS (Executable Test Suite) for WMS 1.3.0
# using Docker with TEAM Engine and the REST API.
#
# Usage:
#   ./run_wms_compliance.sh                              # Test local WMS (default)
#   ./run_wms_compliance.sh --url http://example.com/wms # Test custom URL
#   ./run_wms_compliance.sh --open                       # Open HTML report after
#
# Requirements:
#   - Docker and Docker Compose
#   - WMS service running on localhost:8080 (or custom URL)
...
# Configuration
TEAMENGINE_PORT=9093
COMPOSE_PROJECT="ogc-wms-test"
DOCKER_COMPOSE_FILE="docker-compose.yml"

# Default test parameters
WMS_URL="${WMS_URL:-http://localhost:8080/wms}"
OPEN_REPORT=false
OUTPUT_DIR="results/wms"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --open)
            ...
            ;;
        --help|-h)
            echo "OGC WMS 1.3.0 Compliance Test Runner"
            echo ""
            echo "Usage: $0 [OPTIONS]"
            echo ""
            echo "Options:"
            echo "  -u, --url URL           WMS endpoint URL (default: http://localhost:8080/wms)"
            echo "  -o, --output DIR        Output directory (default: results/wms)"
            echo "  --open                  Open HTML report in browser when done"
            echo "  -h, --help              Show this help message"
            echo ""
            echo "Requirements:"
            echo "  - Docker and Docker Compose"
            echo "  - WMS service running"
            echo ""
            echo "Examples:"
            echo "  $0                                       # Test local WMS"
            echo "  $0 --url http://myserver:8080/wms        # Test custom WMS"
...
get_capabilities_url() {
    local base_url="$1"
    if [[ "$base_url" == *"?"* ]]; then
        echo "${base_url}&SERVICE=WMS&REQUEST=GetCapabilities&VERSION=1.3.0"
    else
        echo "${base_url}?SERVICE=WMS&REQUEST=GetCapabilities&VERSION=1.3.0"
    fi
}
...
check_wms() {
    local caps_url
    caps_url=$(get_capabilities_url "$WMS_URL")
    
    log_info "Checking WMS at ${WMS_URL}..."
    
    local response
    response=$(curl -sf -w "%{http_code}" -o /tmp/wms-caps-check.xml "$caps_url" 2>/dev/null) || response="000"
    
    if [[ "$response" == "200" ]]; then
        if grep -q "WMS_Capabilities\|WMT_MS_Capabilities" /tmp/wms-caps-check.xml 2>/dev/null; then
            log_success "WMS is accessible and returns valid capabilities"
            return 0
        fi
    fi
    
    log_error "WMS is not accessible at ${WMS_URL}"
    log_error "HTTP response code: $response"
    log_error ""
    log_error "Make sure the WMS is running. Start it with:"
...
start_containers() {
    ...
    docker compose -f "$DOCKER_COMPOSE_FILE" -p "$COMPOSE_PROJECT" --profile wms up -d 2>/dev/null
...
stop_containers() {
    ...
    docker compose -f "$DOCKER_COMPOSE_FILE" -p "$COMPOSE_PROJECT" --profile wms down 2>/dev/null || true
}

# Run the tests via REST API
run_tests() {
    log_header "Running OGC WMS 1.3.0 Compliance Tests"
    
    mkdir -p "$OUTPUT_DIR"
    
    # Build capabilities URL for TEAM Engine
    # Use the nginx proxy IP (172.28.0.10) which rewrites localhost URLs in responses
    # This allows TEAM Engine to follow URLs from the Capabilities document
    local caps_url="http://172.28.0.10:8080/wms?SERVICE=WMS&REQUEST=GetCapabilities&VERSION=1.3.0"
    local encoded_url
    encoded_url=$(python3 -c "import urllib.parse; print(urllib.parse.quote('$caps_url', safe=''))")
    
    log_info "Target URL: ${WMS_URL}"
    ...
    if ! curl -sf -u ogctest:ogctest --max-time 300 \
        "http://localhost:${TEAMENGINE_PORT}/teamengine/rest/suites/wms13/run?capabilities-url=$encoded_url" \
...
<head>
    ...
    <title>OGC WMS 1.3.0 Compliance Report</title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .card h2 { font-size: 18px; color: #2d5016; margin-bottom: 15px; border-bottom: 2px solid #e2e8f0; padding-bottom: 10px; }
...
<body>
    <div class="container">
        <div class="header">
            <h1>OGC Web Map Service 1.3.0</h1>
        ...
        <div class="card">
            ...
            <table class="info-table">
                <tr><td>Test Suite</td><td>OGC WMS 1.3.0 ETS (TEAM Engine)</td></tr>
                <tr><td>Implementation</td><td><a href="${WMS_URL}">${WMS_URL}</a></td></tr>
...
print_summary() {
    echo ""
    echo "=========================================="
    echo "       WMS Compliance Test Summary"
    echo "=========================================="
    echo ""
...
main() {
    echo ""
    echo "================================================"
    echo "      OGC WMS 1.3.0 Compliance Test Suite"
    echo "================================================"
    echo ""
