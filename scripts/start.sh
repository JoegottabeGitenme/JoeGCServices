...
show_compose_status() {
    ...
    echo "  Web Dashboard: http://localhost:8000  (Running in docker-compose ✓)"
    echo "  WMS API:       http://localhost:8080  (Start with: cargo run --bin wms-api)"
...
show_compose_access_info() {
    ...
    echo ""
    echo "The web dashboard is running! Open your browser:"
    ...
    echo ""
    echo "Web Dashboard (Interactive WMS Testing - Running):"
    echo "  URL:  http://localhost:8000"
    echo ""
    echo "WMS API (Start in another terminal):"
    echo "  URL:  http://localhost:8080"
    echo "  Cmd:  cargo run --bin wms-api"
    echo ""
    echo "PostgreSQL:"
    ...
    case "${1:-}" in
        ...
        --help|-h)
            ...
            echo ""
             ...
             echo "  1. Run: ./start.sh"
             echo "     This starts all services including the web dashboard"
             echo ""
             echo "  2. Open your browser:"
             echo "     http://localhost:8000  (Web Dashboard)"
             echo ""
             echo "  3. In another terminal, start the WMS API:"
             echo "     cargo run --bin wms-api"
             echo ""
             echo "  4. The WMS API will be available at:"
             echo "     http://localhost:8080"
             echo ""
             echo "  5. Test from command line:"
             echo "     curl 'http://localhost:8080/wms?SERVICE=WMS&REQUEST=GetCapabilities'"
             echo ""
