...
./scripts/start.sh --stop       # Stop containers
...
| Issue | Solution |
|-------|----------|
| Port conflict | Change port in `docker-compose.override.yml` |
...
1. **Fast iteration**: Use `docker-compose up` + `cargo run` for quick development
2. **Debugging**: `docker-compose logs -f` is your friend
