...
impl IngesterConfig {
    ...
    pub fn from_env() -> Result<Self> {
        ...
        models.insert("gfs".to_string(), ModelConfig {
            name: "GFS".to_string(),
        ...
        models.insert("hrrr".to_string(), ModelConfig {
            name: "HRRR".to_string(),
            source: DataSource::NoaaAws {
...
impl DataSource {
    ...
    pub fn build_path(&self, date: &str, cycle: u32) -> String {
        match self {
            DataSource::NoaaAws { prefix_template, .. } => {
                prefix_template
                    .replace("{date}", date)
                    .replace("{cycle}", &format!("{:02}", cycle))
            }
            DataSource::Nomads { path_template, .. } => {
                path_template
                    .replace("{date}", date)
                    .replace("{cycle}", &format!("{:02}", cycle))
            }
    ...
    pub fn file_pattern(&self, model: &str, resolution: &str, cycle: u32, fhr: u32) -> String {
        match self {
            DataSource::NoaaAws { .. } | DataSource::Nomads { .. } => {
                match model {
                    "gfs" => format!(
                        "gfs.t{:02}z.pgrb2.{}.f{:03}",
                        cycle, resolution, fhr
                    ),
                    "hrrr" => format!(
