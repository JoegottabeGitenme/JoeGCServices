...
use grib2_parser::{unpack_simple, Grib2Reader};
...
fn test_roundtrip_gfs_synthetic() {
    ...
    assert_eq!(
        msg.indicator.discipline, 0,
        "Discipline should be meteorological"
    );
    ...
    for (i, (input, output)) in input_values.iter().zip(output_values.iter()).enumerate() {
        ...
        assert!(
            diff < 1.0,
            "Value {} differs too much: input={}, output={}, diff={}",
            i,
            input,
            output,
            diff
...
fn test_roundtrip_mrms_synthetic() {
    ...
    assert_eq!(
        msg.indicator.discipline, 209,
        "Discipline should be MRMS local"
    );
    ...
    for (i, (input, output)) in input_values.iter().zip(output_values.iter()).enumerate() {
        ...
        assert!(
            diff < 1.0,
            "Value {} differs too much: input={}, output={}",
            i,
            input,
            output
        );
...
fn test_grib2_structure_validity() {
    ...
    assert_eq!(
        msg_len as usize,
        grib_bytes.len(),
        "Message length should match actual length"
    );

    // Check end marker
    assert_eq!(
        &grib_bytes[grib_bytes.len() - 4..],
        b"7777",
        "Should end with 7777"
    );
    ...
    assert!(
        section_nums.contains(&1),
        "Should have Section 1 (Identification)"
    );
    assert!(
        section_nums.contains(&3),
        "Should have Section 3 (Grid Definition)"
    );
    assert!(
        section_nums.contains(&4),
        "Should have Section 4 (Product Definition)"
    );
    assert!(
        section_nums.contains(&5),
        "Should have Section 5 (Data Representation)"
    );
...
fn test_compare_synthetic_with_local_gfs() {
    ...
    // Compare key fields
    assert_eq!(
        syn_msg.indicator.discipline, local_msg.indicator.discipline,
        "Discipline should match"
    );
    assert_eq!(
        syn_msg.identification.center, local_msg.identification.center,
        "Center should match"
    );
    assert_eq!(
    ...
    assert_eq!(
        syn_msg.grid_definition.num_points_latitude, local_msg.grid_definition.num_points_latitude,
...
fn test_grib_crate_can_parse_synthetic() {
    ...
    for (_idx, submsg) in grib_file.iter() {
        ...
        assert_eq!(
            indicator.total_length as usize,
            grib_bytes.len(),
            "Length should match"
        );
        ...
        match grib::Grib2SubmessageDecoder::from(submsg) {
            ...
            Err(e) => println!(
                "⚠ Decoder creation failed (structure may still be valid): {:?}",
                e
            ),
        }
    }
...
fn test_constant_value_encoding() {
    ...
    assert_eq!(
        msg.data_representation.bits_per_value, 0,
    ...
    assert!(
        (ref_val - constant).abs() < 0.01,
        "Reference value {} should equal constant {}",
        ref_val,
        constant
    );
