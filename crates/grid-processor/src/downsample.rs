...
pub fn downsample_2x(
    ...
    for out_y in 0..new_height {
        for out_x in 0..new_width {
            ...
            let v10 = data
                .get(in_y * width + in_x + 1)
                .copied()
                .unwrap_or(f32::NAN);
            let v01 = data
                .get((in_y + 1) * width + in_x)
                .copied()
                .unwrap_or(f32::NAN);
            let v11 = data
                .get((in_y + 1) * width + in_x + 1)
                .copied()
                .unwrap_or(f32::NAN);
...
mod tests {
    ...
    fn test_downsample_method_for_parameter() {
        assert_eq!(
            DownsampleMethod::for_parameter("TMP"),
            DownsampleMethod::Mean
        );
        assert_eq!(
            DownsampleMethod::for_parameter("RH"),
            DownsampleMethod::Mean
        );
        assert_eq!(
            DownsampleMethod::for_parameter("REFL"),
