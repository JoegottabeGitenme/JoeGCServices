...
async fn test_chunked_grid_cross_chunk_read() {
    ...
    let valid_values: Vec<f32> = region
        .data
        .iter()
        .filter(|v| !v.is_nan())
        .copied()
...
async fn test_multiscale_metadata() {
    ...
    assert!(
        !ms_array.is_empty(),
        "Should have at least one multiscale entry"
    );
    ...
    assert!(
        datasets_array.len() >= 2,
        "Should have at least 2 pyramid levels"
    );
}
...
async fn test_multiscale_read_levels() {
    ...
    assert_eq!(
        metadata1.shape,
        (32, 32),
        "Level 1 should be 32x32 (half of level 0)"
    );
    ...
    assert_eq!(
        metadata2.shape,
        (16, 16),
        "Level 2 should be 16x16 (quarter of level 0)"
    );
}
...
async fn test_repeated_reads_work() {
    ...
    let region1 = processor
        .read_region(&bbox)
        .await
        .expect("Failed to read first time");
    let region2 = processor
        .read_region(&bbox)
        .await
        .expect("Failed to read second time");
    ...
    for (a, b) in region1.data.iter().zip(region2.data.iter()) {
        if !a.is_nan() && !b.is_nan() {
            assert!(
                (a - b).abs() < 0.001,
                "Data should be identical across reads"
            );
        }
    }
}
