            ...
            if shm_path.exists() {
                group.bench_with_input(
                    ...
                    |b, data| {
                        b.iter(|| {
                            let temp_file =
                                shm_path.join(format!("bench_goes_{}.nc", std::process::id()));
        ...
        group.bench_with_input(
            ...
            |b, data| {
                b.iter(|| {
                    ...
                    let temp_file =
                        temp_dir.join(format!("bench_opt_{}_{}.nc", std::process::id(), count));
...
fn bench_netcdf_io_pattern(c: &mut Criterion) {
    ...
    group.bench_function("sequential_3x_operations", |b| {
        b.iter(|| {
            ...
            for i in 0..3 {
                let temp_file =
                    temp_dir.join(format!("bench_goes_{}_{}.nc", std::process::id(), i));
...
fn bench_goes_resampling(c: &mut Criterion) {
    ...
    for (out_w, out_h, bbox, name) in scenarios {
        ...
        group.bench_with_input(
            ...
            |b, data| {
                b.iter(|| {
                    resample_bilinear_only(black_box(data), goes_width, goes_height, out_w, out_h)
                });
            },
        );
...
fn render_goes_ir(data: &[f32], width: usize, height: usize) -> Vec<u8> {
    ...
    for (i, &temp) in data.iter().enumerate() {
        ...
        let (r, g, b) = if norm < 0.2 {
            ...
            let t = norm / 0.2;
            ((255.0 * (1.0 - t * 0.3)) as u8, 255, 255)
        } else if norm < 0.5 {
            // Cold: cyan to green
            let t = (norm - 0.2) / 0.3;
            ((180.0 * (1.0 - t)) as u8, 255, (255.0 * (1.0 - t)) as u8)
        } else if norm < 0.7 {
            // Mid: green to yellow
            let t = (norm - 0.5) / 0.2;
            ((255.0 * t) as u8, 255, 0)
...
criterion_group!(io_benches, bench_temp_file_io, bench_netcdf_io_pattern,);

criterion_group!(projection_benches, bench_geo_to_grid, bench_geo_to_scan,);

criterion_group!(resample_benches, bench_goes_resampling,);

criterion_group!(color_benches, bench_goes_color_mapping,);
